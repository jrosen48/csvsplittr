...
Here is a short example using a not-too-large, but still hefty CSV file, with [data from Donors Choose](https://data.donorschoose.org/). It is about 800 MB. Let's say we wanted to split it into four files, each about 200 MB.

```{r}
library(csvsplittr)
split_csv("~/downloads/donors_choose_data.csv", 4)
```
